...
int (*matrix_ap)[dbj_c_matrix_sampler_R][dbj_c_matrix_sampler_C] 
= malloc(sizeof(int[dbj_c_matrix_sampler_R][dbj_c_matrix_sampler_C]));

// this is the array pointer  to the first "array of arrays"
...
//////////////////////////////////////////////////
static void matrix_vmt_print 
(MATRIXHANDLE mx, unsigned R, unsigned C ) 
{
    MATRIXTYPE(char,R,C) matrix ;
...
static void * matrix_vmt_foreach 
(MATRIXHANDLE mx, unsigned R, unsigned C, array_user cback ) 
{
    MATRIXTYPE(char,R,C) matrix ;
    matrix * new_mx =  (matrix*)mx ;
    matrix   mx1 = *new_mx ;
    ...
    for (int j = 0; j < len ; j++) cback( cp, j );
