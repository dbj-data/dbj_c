...
#error  _CHAR_UNSIGNED not defined?  /J (Default char Type Is unsigned)
#endif


inline bool is_empty(const char* text) noexcept
{
	return text == 0 || *text == '\0';
}

inline bool wis_empty(const wchar_t* text) noexcept
{
...
__attribute__((const)) char *str_ncpy(char *, const char *, size_t) noexcept;

__attribute__((const)) inline char *str_ncpy(char *destination, const char *source, size_t num) noexcept
{
...
__attribute__((const)) inline void *mem_set(void *s, int c, size_t n)
{
	assert(s);
...
__attribute__((const)) inline int mem_cmp(const void *s1, const void *s2, size_t n)
{
	assert(s1);
	assert(s2);
...
__attribute__((const)) inline void *mem_cpy(void *dest, const void *src, size_t n)
{
	assert(dest);
	assert(src);
...
__attribute__((const)) inline void *mem_chr(const void *s, int c, size_t n)
{
	assert(s);
...
//
__attribute__((const)) inline bool str_equal(char const *lhs, char const *rhs) noexcept
{
	assert(!is_empty(lhs));
	assert(!is_empty(rhs));
...
__attribute__((const)) inline bool wstr_equal(wchar_t const *lhs, wchar_t const *rhs) noexcept
{
...
__attribute__((const)) inline size_t str_len(const char *ch) noexcept
{
	assert(!is_empty(ch));
...
__attribute__((const)) inline size_t wstr_len(const wchar_t *ch) noexcept
{
	assert(!wis_empty(ch));
...
__attribute__((const)) size_t str_nlen(const char *str, size_t max)
{
	assert(!is_empty(str));
...
/*
	...
	*/
__attribute__((const)) auto remove_ws_max_input_size{1024U};

template <typename C, size_t N>
__attribute__((const)) auto remove_ws(const C (&literal_)[N])
{
...
/*
	...
	*/
template <size_t N>
__attribute__((const)) inline std::array<char, N> to_array(const char (&sl_)[N]) noexcept
{
...
template <size_t N>
__attribute__((const)) inline std::array<char, N> to_array(const char *) noexcept = delete;
...
template <typename... T>
__attribute__((const)) inline bool equal3(const T &...args_)
{
	if ((sizeof...(args_)) < 2)
		return true;
...
// simpler ?
inline auto eq3 = [](auto v1, auto v2, auto v3) __attribute__((const)) -> bool
{
	return (v1 == v2) && (v2 == v3);
};
