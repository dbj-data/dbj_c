...
#endif // __clang__
/* 
   (c) by dbj at dbj dot org -- https://dbj.org/license_dbj

   the concept is of course not mine, just the code
   the valstat call/reply protocol is also mine ;)
   
   To use, in exactly one C or CPP file do:

   #define DBJ_FIBO_FACT_IMPLEMENT // see the one function test bellow; optional
   #define DBJ_FIBO_FACT_IMPLEMENT
   #include <dbj_dbj_fibo_fact.h>

   Elsewhere, just include and use
*/
#include <assert.h>
#include <errno.h>
#include <stddef.h>
...
extern "C"
{
  ...
  typedef int64_t dbj_fibo_type;

  /*
int64_t dbj_fibo(int64_t n)
{
    int64_t c = 0, a = 1, b = 1;
...
int64 can hold dbj_fibonacci(N) up to N = 64
...
*/
  ...
  valstat_dbj_fibo dbj_fibo(dbj_fibo_type n);

#ifdef DBJ_FIBO_FACT_IMPLEMENT

  static int64_t dbj_fibo_table_64[] = {1,
                                        1,
                                        1,
                                        2,
                                        3,
...
// 94
#define dbj_fibo_table_size (sizeof(dbj_fibo_table_64) / sizeof(dbj_fibo_table_64[0]))
#define dbj_fibo_min_n 0
#define dbj_fibo_max_n dbj_fibo_table_size - 1

  // the outcome is very safe and simple dbj_fibonacci
  valstat_dbj_fibo dbj_fibo(dbj_fibo_type n)
  {
    if ((n < 0) || (n > dbj_fibo_max_n))
      return (valstat_dbj_fibo){.status = EINVAL};

    return (valstat_dbj_fibo){.value = dbj_fibo_table_64[n]}; // OK
  }
...
#define TEST_BEGIN fprintf(stderr, "\nbegin test:%s()\nfile: %s\nline:%d\n", __func__, __FILE__, __LINE__)

  /* ------------------------------------------------------------------ */
  static void test_dbj_fibo_facto(void)
  {
    TEST_BEGIN;
    // FIBONACCI
    {
      valstat_dbj_fibo vs = dbj_fibo(-1);
      assert(vs.status == EINVAL);
    }
    {
      valstat_dbj_fibo vs = dbj_fibo(dbj_fibo_table_size);
      assert(vs.status == EINVAL);
    }
    {
      valstat_dbj_fibo vs = dbj_fibo(dbj_fibo_max_n);
      assert(vs.status == 0);
